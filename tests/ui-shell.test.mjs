...
test('both theme files carry the generated UI shell block with the same selectors', () => {
  ...
  // The generated block is written from one source for both themes: same rules, in the
  // same order, with only the token values differing.
  // from the end of the header comment to the closing marker
  const block = css => css.slice(css.indexOf('*/', css.indexOf('=== UI shell')) + 2, css.lastIndexOf('/*', css.indexOf('=== /UI shell ===')));
  const shellRules = css => parseStylesheet(block(css)).map(rule => [rule.at, rule.selectors.join(', ')]);
  assert.deepEqual(shellRules(light), shellRules(dark));
  assert.ok(shellRules(dark).length > 50, 'the block was found, not an empty slice');
  const tokens = css => declarationsFor(parseStylesheet(block(css)), ':root');
  assert.notEqual(tokens(dark).get('--saa-bg-app'), tokens(light).get('--saa-bg-app'));
  assert.notEqual(tokens(dark).get('--saa-text'), tokens(light).get('--saa-text'));
});
