...
test('switching the segment patches only the existing keys and restores the last backend', () => {
  ...
  assert.deepEqual(applyAiMode('expand', { ai_interface: 'None', ai_prompt_role: 2 }, 'Pod'), { ai_interface: 'Pod', ai_local_prompt_mode: 'Expand' });
...
test('status line', () => {
  ...
  assert.equal(describeAiStatus({ ai_interface: 'Pod', ai_local_model_mode: 'Auto' }), 'Pod · Auto');
});
